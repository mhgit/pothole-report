...
def test_cli_requires_folder(_mock_keyring: object, temp_config: Path) -> None:
    ...
    with (
        patch.object(sys, "argv", ["report-pothole", "-c", str(temp_config)]),
...
def test_cli_empty_folder(
    ...
    with (
        ...
        patch("pothole_report.cli.Console", return_value=mock_console),
    ):
        main()
...
def test_cli_multi_select_location(
    ...
    with (
        ...
        patch("pothole_report.cli.Console", return_value=mock_console),
        # SystemExit is expected if no GPS/images, but validation should pass
        suppress(SystemExit),
    ):
        main()
...
def test_cli_setup_stores_email(mock_set_password: object) -> None:
    ...
    with (
        patch.object(sys, "argv", ["report-pothole", "setup"]),
        patch("pothole_report.cli.Console") as mock_console_cls,
    ):
        mock_console = MagicMock()
...
def test_cli_remove_keyring_calls_delete(mock_delete_password: object) -> None:
    ...
    with (
        patch.object(sys, "argv", ["report-pothole", "remove-keyring"]),
        patch("pothole_report.cli.Console") as mock_console_cls,
    ):
